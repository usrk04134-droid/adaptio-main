...
TEST_SUITE("Test Snake") {
  TEST_CASE("FromImage - close to edge") {
    auto grayscale_image = imread("./src/scanner/joint_model/test/test_data/1755001276997.tiff", cv::IMREAD_GRAYSCALE);
  ...
  TEST_CASE("FromImage - black image") {
    ...
    auto grayscale_image = imread("./src/scanner/joint_model/test/test_data/1754561083373.tiff", cv::IMREAD_GRAYSCALE);

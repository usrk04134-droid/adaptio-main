...
// ATTENTION: The size of TRACE_CIRCLE_BUFFER_SIZE must be the same as TRACE_AREA_SIZE (in pntrc_circ_trace.h) minus 40
// Bytes!
...
LONG WINAPI pnd_unhandled_exception_filter(struct _EXCEPTION_POINTERS* ep) {
  ...
  if ((!isFatalOccured))  // First one ?
  {
    isFatalOccured = TRUE;
    char reason[2048];
    sprintf(reason, "Unhandled exception - Code(0x%08x) - Address(0x%08x)", ep->ExceptionRecord->ExceptionCode,
            (PNIO_UINT32)ep->ExceptionRecord->ExceptionAddress);
  ...
  if (PNTRC_fp != 0) {
    ...
    if (MAX_NUM_OF_PND_TRACE_FILES != 0 && MAX_NUM_OF_PND_TRACE_FILES != 1) {
      ...
      if (file_size >= MAX_SIZE_OF_PND_TRACE_FILE) {
        ...
        /*
         * MAX_NUM_OF_PND_TRACE_FILES defines maximum number of "pnd_trace.bin" files that will be preserved for
         * instance if MAX_NUM_OF_PND_TRACE_FILES defined to be as 3, in a specific moment of time there will 4 instance
         * of log files that is, "pnd_trace.bin", "pnd_trace.bin_01", "pnd_trace.bin_02" and "pnd_trace.bin_03"  in this
         * configuration the current file where the traces are logged will be file "pnd_trace.bin", and the oldest trace
         * file will  be "pnd_trace.bin_03" when the size of file "pnd_trace.bin" exceeds MAX_SIZE_OF_PND_TRACE_FILE,
         * file "pnd_trace.bin_03" will be deleted, file "pnd_trace.bin_02" will be renamed as "pnd_trace.bin_03" file
         * "pnd_trace.bin_01" will be renamed as "pnd_trace.bin_02", file "pnd_trace.bin" will be renamed as
         * "pnd_trace.bin_01". file "pnd_trace.bin" will be reopened with size 0 and new traceses will continue to be
         * logged to "pnd_trace.bin"
         */

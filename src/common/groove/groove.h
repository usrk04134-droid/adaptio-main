...
inline constexpr int ABW_POINTS = 7;
inline constexpr int ABW_UPPER_LEFT = 0;
inline constexpr int ABW_LOWER_LEFT = 1;
inline constexpr int ABW_LOWER_RIGHT = 5;
inline constexpr int ABW_UPPER_RIGHT = 6;
...
class Groove {
 public:
  ...
  auto Area() const -> double { return common::groove::PolygonArea({groove_.begin(), groove_.end()}); }
  ...
  auto BottomWidth() const -> double {
    return (groove_.at(ABW_LOWER_LEFT) - groove_.at(ABW_LOWER_RIGHT)).horizontal;
  }
